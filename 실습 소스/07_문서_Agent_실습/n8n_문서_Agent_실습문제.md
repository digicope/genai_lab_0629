...
```
Webhook → Edit Fields → HTTP Request — Document Parse → Extract Markdown → Respond to Webhook
```

| 노드 | 설정 |
|------|------|
| **HTTP Request — Document Parse** | Method: `POST`, URL: `https://api.upstage.ai/v1/document-digitization` |
| | Authentication: **Header Auth** (Name: `Authorization`, Value: `Bearer {API_KEY}`) |
...
- [ ] **Extract Markdown** 노드 출력에 `markdown`이 있는가?
- [ ] **Header Auth** Credential이 연결되었는가?

### 힌트

- Edit Fields 노드는 **binary 데이터(`file`)를 유지**합니다.
